...
def test_settings_use_lite_defaults(monkeypatch):
    monkeypatch.setenv("SLYNK_DEFAULT_FILE_TTL_HOURS", "8")
    monkeypatch.setenv("SLYNK_PENDING_SESSION_STALE_HOURS", "24")
    ...
    monkeypatch.setenv("SLYNK_PRIVATE_BEARER_TOKEN", "private-token")

    settings = Settings()

    assert settings.DEFAULT_FILE_TTL_HOURS == 8
    assert settings.PENDING_SESSION_STALE_HOURS == 24
    ...
    assert settings.PRIVATE_BEARER_TOKEN == "private-token"
